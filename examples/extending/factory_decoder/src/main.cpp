...
WARNING: If you create a derived data type you will also need to ensure that it
is correctly padded. See the DIS standard for specifics.
...
//
// This is our custom class, it will be used to decode any data that has the
// DatumID '500123'
//
class MyCustomClass : public VariableDatum {
 ...
 public:
  explicit MyCustomClass(KUINT64 Val) {
    m_ui32DatumID = 500123;  // Our custom DatumID.
    m_ui32DatumLength =
        64;  // The Datum length is the size in bits of our custom data
    m_ui64OurTestValue = Val;
  }

  explicit MyCustomClass(KDataStream& stream) { Decode(stream); }
  ...
  virtual KString GetAsString() const {
    ...
    ss << "This is our custom VariableDatum!!!! Our value is "
       << m_ui64OurTestValue << "\n";
...
class MyDecoderClass : public FactoryDecoder<VariableDatum> {
 public:
  ...
  virtual VariableDatum* FactoryDecode(KINT32 EnumVal, KDataStream& stream) {
    ...
    // We could use a single decoder class to support multiple custom data
    // types....
    if (EnumVal == 500123) {
...
int main() {
  try {
    ...
    if (pdu.get()) {
      ...
      MyCustomClass* pMyClass =
          dynamic_cast<MyCustomClass*>((VariableDatum*)pVd);
