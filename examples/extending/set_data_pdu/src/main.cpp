...
Example created by request:
https://sourceforge.net/p/kdis/discussion/879756/thread/89a6e27d/
...
// An example custom Variable Datum that contains a Entity ID and World
// Coordinates Record
class TargetPosition : public VariableDatum {
 ...
 public:
  ...
  TargetPosition(EntityIdentifier id, WorldCoordinates wc)
      : m_EntID(id), m_worldCoords(wc), m_ui16padding(0) {
    m_ui32DatumID = 12345;  // Our DatumID. Pick something to use that is not
                            // already in the DatumID enum.

    // Add the length of our fields on
    m_ui32DatumLength =
        EntityIdentifier::ENTITY_IDENTIFER_SIZE *
        8;  // The Datum length is the size in BITS of our custom data
    m_ui32DatumLength += WorldCoordinates::WORLD_COORDINATES_SIZE * 8;
  }

  // Ctor
  TargetPosition(KDataStream& stream) { Decode(stream); }
  ...
  virtual KString GetAsString() const {
    ...
    ss << "This is our custom VariableDatum!!!!\n"
       << m_EntID.GetAsString() << m_worldCoords.GetAsString();
  ...
  virtual void Decode(KDataStream& stream) {
    ...
    stream >> KDIS_STREAM m_EntID >> KDIS_STREAM m_worldCoords >>
        m_ui16padding;  // The variable datum needs to be padded to a 64 bit
                        // boundary, with our 2 new fields the total size is 38
                        // bytes, we need 2 more bytes to be on a 64 bit
                        // boundary(40).
  }
  ...
  virtual void Encode(KDataStream& stream) const {
    ...
    stream << KDIS_STREAM m_EntID << KDIS_STREAM m_worldCoords
           << m_ui16padding;  // The variable datum needs to be padded to a 64
                              // bit boundary, with our 2 new fields the total
                              // size is 38 bytes, we need 2 more bytes to be on
                              // a 64 bit boundary(40).
  }
};

//
// Our decoder class. This tells KDIS how to decode a specific DatumID. So when
// the DatumID is 12345 we want to create a TargetPositionRecord
//
...
int main() {
  try {
    // 1
    // Register our factory decode function, KDIS will use this decoder when it
    // encounters a VariableDatum record that has this DatumID
    VariableDatum::RegisterFactoryDecoder(
        12345, VariableDatum::FacDecPtr(new TargetPositionDecoderClass));

    // 2
    // Create an instance of our data type.
    TargetPosition* pTargetPos = new TargetPosition(EntityIdentifier(1, 2, 3),
                                                    WorldCoordinates(3, 2, 1));
    ...
    // 5
    // Lets skip actually sending this data over the network in order to keep
    // things simple..
    //
    ...
    if (pdu.get()) {
      ...
      TargetPosition* pMyRecvTargetPos =
          dynamic_cast<TargetPosition*>((VariableDatum*)pVd);
  ...
  // 7
  // Now we should clean up memory. Our decoder will not be deleted unless we
  // call ClearFactoryDecoders.
