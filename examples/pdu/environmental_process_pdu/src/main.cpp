...
This example shows how to correctly use the Environmental_Process_PDU to create
a point feature in our simulation which will represent a cloud.
*********************************************************************/
...
int main() {
  try {
    ...
    ourEnvProcPDU.SetEnvironmentalProcessID(EntityIdentifier(
        1, 123, 1));  // The ID of our application (SiteID, AppID, ObjectID)
    ourEnvProcPDU.SetEnvironmentType(
        EnvironmentType(Air, 0, 3, SmallSize, 0,
                        0));  // This value is taken from section 4 of the
    ...
    ourEnvProcPDU.AddEnvironmentRecord(
        EnvironmentRecordPtr(new PointRecord1(0, loc)));
