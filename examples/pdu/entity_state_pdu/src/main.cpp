...
This example shows the older method for sending a PDU, to see how the connection
class can further help see the Environmental_Process_PDU example.
*********************************************************************/
...
int main() {
  try {
    ...
    KDIS::UTILS::GeodeticToGeocentric(Lat, Lon, Alt, GeoX, GeoY, GeoZ,
                                      WGS_1984);
    KDIS::UTILS::HeadingPitchRollToEuler(DegToRad(Heading), DegToRad(Pitch),
                                         DegToRad(Roll), DegToRad(Lat),
                                         DegToRad(Lon), Psi, Theta, Phi);
    ...
    Entity_State_PDU Entity(EntID, EntForceID, EntType, EntType,
                            EntityLinearVelocity, EntityLocation,
