...
This example is the same as PDU Factory example 1 however it uses some of the
new features provided by the Connection class.
...
//
// This is our example subscriber class. It simply prints out the senders IP
// address and the contents of the PDU.
//
class ExampleSubscriber : public ConnectionSubscriber {
 public:
  ...
  virtual KBOOL OnDataReceived(const KOCTET* Data, KUINT32 DataLength,
    ...
    // We return true here, if we return false then the data will be discarded
    // by the connection class. This means an IP address based filter could be
    // implemented here.
    return true;
  }
...
int main() {
  try {
    ...
    // Lets apply a filter to the factory so it only lets through PDU that have
    // an exercise ID of 1.
