...
TEST(utils, BoundedLengthString) {
  KDIS::UTILS::BoundedLengthString<7> bls;
  ...
  KDIS::UTILS::BoundedLengthString<7> bls2;
...
TEST_F(NumberConversionsTest, HandlesNaN) {
  EXPECT_THROW(KDIS::UTILS::double_to_int32(nan), std::invalid_argument);
}

TEST_F(NumberConversionsTest, HandlesPositiveInfinity) {
  EXPECT_THROW(KDIS::UTILS::double_to_int32(infinity), std::invalid_argument);
}
...
TEST_F(NumberConversionsTest, ThrowsOnOverflow) {
  ...
  EXPECT_THROW(KDIS::UTILS::double_to_int32(too_big), std::overflow_error);
  EXPECT_THROW(KDIS::UTILS::double_to_int32(1e10), std::overflow_error);
}

TEST_F(NumberConversionsTest, ThrowsOnUnderflow) {
  constexpr double too_small{int_min_as_double - 1.0};
  EXPECT_THROW(KDIS::UTILS::double_to_int32(too_small), std::underflow_error);
  EXPECT_THROW(KDIS::UTILS::double_to_int32(-1e10), std::underflow_error);
}

TEST_F(NumberConversionsTest, TruncatesPositiveValues) {
  EXPECT_EQ(3, KDIS::UTILS::double_to_int32(3.7));
  EXPECT_EQ(3, KDIS::UTILS::double_to_int32(3.2));
  EXPECT_EQ(std::numeric_limits<std::int32_t>::max(),
            KDIS::UTILS::double_to_int32(int_max_as_double));
  EXPECT_EQ(1, KDIS::UTILS::double_to_int32(1.5001));
}
...
TEST_F(NumberConversionsTest, TruncatesNegativeValues) {
  ...
  EXPECT_EQ(std::numeric_limits<std::int32_t>::min(),
            KDIS::UTILS::double_to_int32(int_min_as_double));
  EXPECT_EQ(-1, KDIS::UTILS::double_to_int32(-1.5001));
}

TEST_F(NumberConversionsTest, HandlesZero) {
  EXPECT_EQ(0, KDIS::UTILS::double_to_int32(0.0));
  EXPECT_EQ(0, KDIS::UTILS::double_to_int32(-0.0));
}
